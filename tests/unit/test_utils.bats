...
@test "write_gitignore_template: Pyve-managed section includes .pyve/envs (H.e.2a; renamed in M.h.3)" {
    write_gitignore_template
    run grep -qxF ".pyve/envs" .gitignore
    [ "$status" -eq 0 ]
}
...
@test "write_gitignore_template: idempotent after purge-then-reinit cycle (byte-level)" {
    ...
    # entries but left .pyve/envs (was .pyve/testenv pre-M.h.3), a second
