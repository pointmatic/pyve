...
- **Per-env runnability canary** (Story P.z) — a new optional `env_probe` plugin hook executes a **console-script wrapper** (`bin/pip --version`; micromamba via `micromamba run -p`) in the root env and every declared + materialized named env, classifying the result (`runnable` / `dead-shebang` / `dangling-symlink` / `missing-interpreter` / `broken`; declarative-only and not-materialized envs are silent). A manifest↔disk reconciliation flags **orphans** — materialized trees the manifest doesn't declare, or trees under a declared non-materializable root. Every broken verdict carries the role-correct rebuild verb (root → `pyve init --force`, named → `pyve env init <name> --force`; never `pyve env purge root`). Closes the false green where a relocated env's dead wrappers hid behind a valid `python` symlink.
- **Resolution reasoning in `pyve check`** (Story P.aa) — a `[resolution]` section names, per managed command (`python`, `pip`, `project-guide`), the winning PATH slot (project env / `~/.local/bin` / version-manager shim / system) and probed version, with classified findings when something is wrong: `[venv-pin-drift]` (env frozen to its creation-time interpreter while the pin moved), `[no-version-set]` (a shim rejecting the command under the active pin), `[broken-winner]`, `[not-found]`. `--verbose` prints the full slot-by-slot PATH trace. Findings are warn-at-most narrative — the exit contract is untouched.
- **`pyve check --fix` — self-healing** (Stories P.ab.1, P.ab.2) — a plan-then-confirm heal engine over the probes' verdicts. Non-destructive tier: a provisioned-but-broken toolchain venv is removed and re-provisioned (the provisioning fast paths are presence-gated and used to *skip* a dead venv), a dead hosted project-guide is force-reinstalled, a dangling shim re-linked. Destructive tier: broken root/named envs rebuilt via the role-correct verbs, a venv off the declared pin rebuilt toward it, orphaned trees removed — each **individually confirmed**; `--yes` assents to the prompts; a **non-interactive run never applies destructive repairs** (reported and skipped, so `check --fix --yes` is CI-safe). Idempotent: the plan recomputes from live probes, so re-runs after success report "Nothing to heal."
- **Declarative silent-skip opt-out: `isolated = true`** (Story P.x) — a project that deliberately runs several isolated `purpose = "test"` envs declares it per env in `pyve.toml`; the advisory stays quiet when a marked env is targeted (and still fires for unmarked targets). Project-scoped, reviewable, non-leaky — the per-shell `PYVE_NO_TESTENV_ADVISORY=1` remains as a one-off/CI override. Ships the manifest's first line-attributed validation error (`pyve.toml:<n>: … expected a boolean`).
- **Update-availability hints** (Stories P.ac, P.ad) — Pyve's first network touchpoint, built on an integration spike's measured pattern: bounded silent `curl` (2s connect / 5s total; a network failure can never change the exit code), latest versions from PyPI (project-guide) and the raw Homebrew-tap formula (pyve — deliberately not the GitHub releases API, which reflects the retired pre-merge-flow workflow's last Release), a 24h cache under `$XDG_CACHE_HOME/pyve/latest/`, and hints with the install-source-correct upgrade command. Runs only for interactive human `check` runs: `--offline` / `PYVE_NO_NETWORK=1`, the `CI` env var, and piped output all suppress it.

### Changed

- **`pyve check` gets honest** (Story P.z) — the existence-only root probe and the wrapper-bypassing `python -c 'import pytest'` testenv probe are replaced by the canary. Documented as technically-but-trivially breaking: environments that previously reported a false green now report broken (that was always the contract); the 0/2 exit-code semantics themselves are unchanged.
- **One project-guide readout** (Story P.y) — the v2-era `project-guide:` row in `status`'s `[python]` Integrations block (which checked the project venv and contradicted the real state on every hosted machine) is gone; the `[project-guide]` section is the sole readout, naming *how* the tool is present (`pyve-hosted (toolchain)` / `local pip` / `not installed`) with a **runnability-probed** version — a hosted copy that cannot exec reports `broken (installed but not runnable)` instead of healthy. `pyve self provision` prints the version it installed.
- **Diagnostics docs** (Story P.ae) — the check reference documents the verdict vocabulary and resolution narrative with a worked example from the triggering incident, cross-links "check detects → `--fix` repairs", and corrects the page's stale pre-composition exit-code list.
...
- **`pyve self provision --status [--json]`** (Story O.c) — a read-only, side-effect-free hosting-readiness query other tools (project-guide first) consult **without** a project context or knowledge of Pyve's internal paths. Exit codes: `0` = hosting ready (toolchain venv **and** hosted project-guide both **runnable**), `1` = Pyve-managed but not ready (never provisioned, or provisioned-but-broken), `2` = not Pyve-managed here (the project owns project-guide via a dependency). Classification probes **runnability** — it executes `python --version` / `project-guide --version` rather than stat-ing the artifacts, so a dangling shim or dead-shebang interpreter reads as broken, not "ready". `--json` emits `{ pyve_managed, toolchain:{provisioned,runnable,version}, project_guide:{hosted,runnable,version,shim} }`. The same runnability probe now backs `pyve check`'s `[pyve]` hosting lines, so the human and machine surfaces can never disagree. Honors the `PYVE_PROJECT_GUIDE_BIN` / `PYVE_PYTHON` overrides. This is a cross-repo contract: **project-guide ≥ 2.15.0** consumes it to gate its local-install warning (and never advises removal when hosting isn't ready); the pyve-hosted project-guide install floor was bumped to `>=2.15.0` to match.

### Fixed

- **`pyve self provision <unknown-flag>` silently re-provisioned the whole toolchain and returned `0`** (Story O.c) — the `provision)` dispatcher only special-cased `--help` and fell through to provisioning for *anything else*, so a probe like `pyve self provision --status` against a Pyve too old to know `--status` re-created hosting on every call and always read exit 0 → project-guide read a false "global is active" and emitted destructive `pip uninstall project-guide` advice. The arm now rejects any unrecognized flag with a hard error; **bare `provision` (no args) is the only form that provisions** — the fall-through is impossible by construction.
- **`pyve self migrate` committed the entire `.v2-legacy` backup** (Story O.a) — the composed `.gitignore` emitted anchored `.pyve/envs` + `.pyve/testenvs` patterns that didn't match the nested `.pyve/.v2-legacy/…` (nor `.pyve/bin/…`) trees the migrator creates, and `self migrate` never refreshed `.gitignore` at all — so a migration dragged 1000+ files (the whole moved legacy testenv venv) into the commit. The composer now emits a single whole-tree `.pyve/` ignore (matching the documented "everything under `.pyve/` is materialized state" invariant), and `self migrate` recomposes `.gitignore` right after the backup, unconditionally (covers `--no-rebuild` and rebuild), with a matching `--dry-run` plan line.
- **The emitted `.gitignore` leaked the rationale comment + integration-suite assertions encoded the old contract** (Story O.b, CI follow-up to O.a) — the whole-tree rationale was moved out of the composer heredoc into a source comment (the emitted block is now `.pyve/` only), and four pytest integration assertions were updated from the enumerated-subdir contract to `.pyve/` (one substring check tightened to exact line membership so the managed comment block can't false-pass it).
...
See [docs/specs/phase-m-testenv-dx-plan.md](docs/specs/phase-m-testenv-dx-plan.md) for the plan doc and [docs/specs/phase-m-pyve-named-testenvs.md](docs/specs/phase-m-pyve-named-testenvs.md) for the use-case brief.

### Added

- **Named test environments** (Stories M.f, M.g) — `[tool.pyve.testenvs]` config schema with top-level `default` and per-env tables. Two reserved names: `root` (selection-only, routes pytest to the project's root env) and `testenv` (the implicit-default name). Schema validated at config-read time via a Python TOML helper invoked as a one-shot subprocess; bash-side reader pattern documented in [docs/specs/spike-m-f-testenvs-config.md](docs/specs/spike-m-f-testenvs-config.md). New `lib/testenvs.sh` foundation owns config reading, name resolution, and validation.
- **v2.8 layout** (Story M.h) — every test environment lives at `.pyve/testenvs/<name>/{venv,conda}/` (plural `testenvs/`, name-keyed) with a sibling `.state` file (plain `key=value`: `backend`, `manifest`, `manifest_sha256`, `provisioned_at`, `last_used_at`). Projects upgrading from v2.7 migrate transparently the first time `pyve update` runs or a `pyve test` / `pyve testenv …` invocation needs the testenv; the legacy `.pyve/testenv/` directory is removed once migration completes.
- **Name-aware `testenv` namespace commands** (Story M.i) — `pyve testenv init [<name>]`, `pyve testenv install [<name>] [-r <file>] [--no-wait]`, `pyve testenv purge [<name>] [--force]`, `pyve testenv run [<name> --] <cmd> [args...]`. With `<name>`, operates on one declared env; without, install iterates non-lazy declared envs and purge iterates all declared envs with TTY-aware confirmation. The reserved `root` is rejected by every operational leaf (selection-only); undeclared names hard-error with a `[tool.pyve.testenvs]` hint.
- **Per-env install lock** (Story M.j) — `mkdir`-based atomic lock at `.pyve/testenvs/<name>/.lock/` with `pid` recording. Wait+retry default (1-second sleep, 10-minute cap); `--no-wait` fast-fails with a "(pid N)" message. Stale-lock reclamation via `kill -0 <pid>`. Release is gated on `$$` matching the recorded pid. `trap … EXIT INT TERM` ensures release on any exit path, including `testenv install`'s hard-error paths and SIGINT.
- **Conda-backed testenvs** (Story M.k) — `backend = "micromamba"` (or `backend = "inherit"` resolving to the main env's backend) on any named env provisions via `micromamba create -p <path> -f <manifest> -y`. Independent of the main project backend: a venv main env can carry a conda testenv, or vice versa. `inherit` reads `.pyve/config`'s `backend` field. `pyve testenv run <conda-name>` and `pyve test --env <conda-name>` hard-error with a `micromamba run -p <env-path> <cmd>` workaround hint (PATH-only activation is insufficient for `CONDA_PREFIX`).
- **Venv manifest sources** (Story M.l) — per-env declarations `requirements = ["a.txt", "b.txt"]` (one or more pip manifests), `extra = "<name>"` (resolves `[project.optional-dependencies].<name>` via the helper's `--resolve-extra` mode), and `manifest = "<env.yml>"` (conda). Source-dispatch precedence: CLI `-r <file>` → declared `requirements` → declared `extra` → declared `manifest` → auto-detected `requirements-dev.txt` → bare `pytest` fallback. Mutex enforced at config-read time.
- **`pyve test --env <name>`** (Story M.m) — `<name>` accepts any declared name; absent `--env` defaults to `[tool.pyve.testenvs].default` (fallback: `testenv`). Resolver hard-errors on undeclared names (listing every valid choice), conda-backed envs (venv-only `run`), and unprovisioned lazy envs under `PYVE_NO_AUTO_PROVISION=1`. Touches `.state.last_used_at` before exec so `pyve testenv list` / `prune` can see it. `ensure_testenv_exists` and `_testenv_init_conda` write initial `.state` on env creation.
- **Lazy provisioning** (Story M.n) — `lazy = true` opts an env out of `pyve testenv install` (no-arg) iteration. `pyve test --env <lazy-name>` auto-provisions on first targeted use via `ensure_testenv_exists` + `_testenv_install_with_lock`. New `PYVE_NO_AUTO_PROVISION=1` env var restores the M.m hard-error with a `pyve testenv install <name>` hint for strict CI that wants "is this env already built?" semantics.
- **Generalized silent-skip advisory** (Story M.o) — extends the M.c trap-warning to **every** named env. Helper renamed `_test_main_env_has_pytest` → `_test_env_has_pytest <name>`. When `pyve test` routes to env `<T>`, scans `root` + every declared name (skipping `<T>`), lists every alternative env where `import pytest` succeeds. Suppressible via `PYVE_NO_TESTENV_ADVISORY=1` (matrix mode sets this automatically).
- **`pyve testenv list`** (Story M.p) — tabulates the union of declared envs and on-disk envs with `NAME / BACKEND / SIZE / LAST-USED / STATE`. STATE is one of `ready`, `lazy`, `not provisioned`, or `orphaned`. Read-mostly; no mutations.
- **`pyve testenv prune`** (Story M.p) — three disk-walking modes: no args (remove orphans only; declared and reserved-`testenv` are preserved), `--unused-since <YYYY-MM-DD>` (remove envs whose `.state.last_used_at` is strictly older; never-used envs preserved), `--all` (remove every on-disk env). Confirmation gating mirrors `testenv purge` (TTY prompt; `--force` skips; non-TTY skips). Bad date format hard-errors before walking. Distinct from `testenv purge` no-arg: prune is disk-driven, purge is config-driven.
- **`pyve lock --env <name>`** and **`pyve lock --all`** (Story M.q) — extend `lock_environment` to per-testenv conda-lock dispatch. `--env <name>` locks one conda-backed testenv (writes `<manifest>-lock.yml` sibling to the manifest). `--all` locks the main env (in a subshell so its exit-paths don't halt iteration) + every conda-backed testenv. Venv-backed envs are silently skipped under `--all`; hard-error under `--env <venv-name>`. Per-env failures `warn` and accumulate into a non-zero exit; iteration always completes.
- **Matrix execution — `pyve test --env a,b,c`** (Story M.r) — comma-separated `--env` value runs pytest against each env sequentially. Each env's output is preceded by a `=== Env: <name> ===` header. Exit code aggregates worst-case (highest failing rc; 0 only when every env passes); iteration never halts on a failing env. The silent-skip advisory is suppressed inside the matrix loop. `--parallel` is out of scope.
- **`PYVE_NO_AUTO_PROVISION`** env var (Story M.n) — set to `1` to suppress lazy auto-provisioning on `pyve test --env <lazy-name>`. Restores the M.m hard-error with a `pyve testenv install <name>` hint.

### Changed

- **`testenv` namespace commands now accept optional `<name>`** (Stories M.i.2–M.i.4) — `init`, `install`, `purge`, `run` all gained an `[<name>]` positional that defaults to the implicit `testenv`. `run` requires the `--` separator when a name is present (`pyve testenv run smoke -- mypy src/`). No-arg behavior preserved for projects without `[tool.pyve.testenvs]`.
- **`pyve test --env`** (Stories M.m, M.r) — accepts any declared name in addition to reserved `root` / `testenv`, and accepts a comma-separated matrix list. Undeclared name in the list hard-errors with the precise list of valid choices.
- **`pyve testenv install` no-arg** (Story M.i.3) — iterates every declared non-lazy env (in the implicit-default config, that's exactly one). Lazy envs are skipped from iteration; conda-backed envs are dispatched via `_testenv_install_conda` rather than the `M.k` stub.
- **`pyve testenv purge --keep-testenv`** (Story M.h.3) — semantic expansion: now preserves the entire `.pyve/testenvs/` tree (every named env) rather than just the legacy singular `.pyve/testenv/` directory.

### Documentation

- **`docs/site/testing.md`** (Story M.s) — new "Named test environments" section with full config schema, dependency-source matrix, reserved-names table, lazy provisioning, conda-backed envs, `pyve lock --env`/`--all`, matrix execution. Two-env model table, lifecycle (6 leaves), advisory text, and all path references updated for the v2.8 layout.
- **`docs/site/usage.md`** (Story M.s) — `pyve test`, `testenv`, `lock`, and `purge` reference sections rewritten for v2.8 CLI surface.
- **`docs/site/backends.md`** (Story M.s) — per-backend "Testing on …" subsections call out per-env backend independence and `pyve lock --env`/`--all`.
- **`README.md`** (Story M.s) — "The dev/test runner environment" paragraph updated for the v2.8 layout and a one-paragraph pointer to named test environments.
- **`docs/project-guide/templates/artifacts/pyve-essentials.md`** (Stories M.h.4, M.s) — Workflow-rules block updated for "at least two" envs, `--env <name>` / matrix routing, named-env init/install examples, LLM-internal testenv-init guidance, v2.8 layout admonition.
- **`docs/specs/features.md`** (Stories M.h.4, M.m–M.r, M.s) — FR-11 expanded across stories; new **FR-11a: Named Test Environments** as the source-of-truth for the DX surface.
- **`docs/specs/tech-spec.md`** (Stories M.g–M.r) — `lib/testenvs.sh` design, `.state` schema, layout-migration mechanism, conda backend dispatch, `_test_run_one_env` worker, `_lock_*` factoring, all consumer-list bullets.

### Internal

- **`lib/testenvs.sh`** (Story M.g) — shared config reader, env-name resolver, backend validator, `.state` helpers (`state_write` / `state_read` / `state_touch_last_used`), `is_testenv_declared` / `is_testenv_reserved` / `is_testenv_lazy`, `list_testenv_names`, `_testenv_resolve_backend`. The Python TOML helper at `lib/pyve_testenvs_helper.py` emits bash-assignment syntax (not `declare`) so `eval` lands in global scope under bash 3.2's missing-`declare -g` constraint. Helper honors `${PYVE_PYTHON:-python}` for interpreter override.
- **Layout migration mechanism** (Stories M.h.1–M.h.3) — `migrate_legacy_testenv_layout` helper detects `.pyve/testenv/venv/` → moves to `.pyve/testenvs/testenv/venv/` → writes initial `.state`. Wired into `pyve update` (eager) and `resolve_testenv_path testenv` (opportunistic, before returning the new path). Six consumer files swept to read paths via the resolver instead of hard-coded literals; `TESTENV_DIR_NAME` retained as a back-compat constant but unused internally.
- **`test_tests` factored into `_test_run_one_env`** (Story M.r) — per-env body extracted from `test_tests` so the matrix loop can call it in a subshell without losing the single-env exec contract. Matrix dispatcher splits CSV via per-call `IFS=','` (caught the hard way that `local IFS=','` leaks for the rest of the function and corrupts `"$*"` expansions).
- **`lock_environment` factored** (Story M.q) — pre-M.q main-env body extracted into `_lock_main_env`; new `_lock_one_env` / `_lock_env_lock_path` / `_lock_all_conda_testenvs` deliver the per-env surface. `--all` invokes `_lock_main_env` in a subshell so its exit-paths don't abort the conda-testenv iteration.
