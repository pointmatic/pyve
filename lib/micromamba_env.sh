...
# Scaffold a starter `environment.yml` when `pyve init --backend micromamba`
# runs in a directory that has neither an `environment.yml` nor a
# `conda-lock.yml`. The scaffold is intentionally minimal: conda-forge
# channel, the requested Python pin, plus `pip` as a dependency. The
# user edits the file to add their real dependencies before running
...
#   1 if scaffolding was refused (strict, or env.yml / conda-lock.yml already present)
scaffold_starter_environment_yml() {
    ...
    # Never overwrite an existing environment.yml.
    if [[ -f "environment.yml" ]]; then
        return 1
    fi

    # A dangling conda-lock.yml with no environment.yml is an inconsistent
    # state; H.f.6's actionable error handles it. Don't paper over by
    # scaffolding on top.
    if [[ -f "conda-lock.yml" ]]; then
        return 1
    fi

    # Resolve the env name: explicit flag wins; otherwise use the
    # sanitized cwd basename.
    local env_name="$env_name_flag"
    if [[ -z "$env_name" ]]; then
        env_name="$(sanitize_environment_name "$(basename "$PWD")")"
    fi
