...
| `testenv_install` | `(<env_path> <requirements_file?>)` | Pip-install into a venv testenv. Without `<requirements_file>`, installs bare `pytest`. With `<requirements_file>`, validates the file exists, then `pip install -r <file>`. Exits 1 if the testenv doesn't exist (caller must `pyve testenv init` first) or the requirements file is missing. **Conda-backed envs do NOT call this leaf** — backend dispatch in `_testenv_install_with_lock` routes them to `_testenv_install_conda` instead. |
...
- M.l–M.m: venv manifest sources, lazy provisioning.
