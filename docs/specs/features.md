...
3. **Lock file validation (declarative — Story N.bf.8–N.bf.11)** — For micromamba environments, whether a lock is *required* is keyed on the project's own declaration: `conda-lock` present as a dependency in `environment.yml`. When it is declared and `conda-lock.yml` is missing, non-strict `init` proceeds and **nudges** ("run `pyve lock`"); `--strict` **barks** (hard error). When `conda-lock` is *not* declared, no lock is expected and init proceeds silently (pre-production). `--no-lock` opts out for the run (resolve from `environment.yml`, never deletes a committed lock) and beats `--strict`. A stale `conda-lock.yml` warns interactively (or errors in `--strict`). `init` never auto-runs `pyve lock`.
...
- Pyve does not install `conda-lock` as a standalone binary — but since Story N.bf.11 the default micromamba scaffold *declares* `conda-lock` in `environment.yml` (omittable via `--no-lock`), so micromamba installs it into the env as part of normal env creation. Pyve wraps the invocation via `pyve lock` when it is available on PATH.
...
| `--strict` | Enforce the lock requirement (bark on a declared-but-missing/stale lock); also opts out of scaffolding/inference | `pyve init --strict` |
| `--no-lock` | Don't use a lock this run — resolve from `environment.yml`, ignore any present lock (never deletes it), and skip the requirement (beats `--strict`); omit `conda-lock` from a fresh scaffold | `pyve init --no-lock` |
...
**Generated content.** The scaffold pins Python to the resolved `--python-version` (or `DEFAULT_PYTHON_VERSION` if omitted) on the `conda-forge` channel, and adds `pip`. By default it also declares `conda-lock` (Story N.bf.11) so the env can run `pyve lock` immediately — interactive init asks "Version-control dependencies with a lock file? [Y/n]" (default yes), non-interactive init includes it by default, and `--no-lock` omits it:
...
dependencies:
  - python=<version>
  - pip
  - conda-lock      # default; omitted under --no-lock (or wizard "n")
```
...
**Lock-file interaction (declarative model — Story N.bf.8–N.bf.11).** The lock requirement is keyed on the *declarative signal* `is_conda_lock_declared` (is `conda-lock` a dependency in `environment.yml`?), not on a transient `PYVE_NO_LOCK`-on-scaffold flag (retired in N.bf.9). A default scaffold declares `conda-lock`, so `validate_lock_file_status()` recognizes the project as opting into locking but proceeds in non-strict mode and emits the end-of-init **nudge** ("run `pyve lock` when your dependencies are finalized"). A `--no-lock` / wizard-"n" scaffold omits `conda-lock`, so no lock is expected and init is silent. Init still does **not** auto-run `pyve lock` — that would hide dependency-resolution time inside a first run and over-pin a starter env before the user has added real dependencies; the nudge makes the manual step discoverable instead.
...
- Does **not** add opinionated dependencies beyond `python`, `pip`, and (by default) `conda-lock`. `pyve lock` solves the rest; the user adds their real dependencies before locking.
...
- **Prerequisite check**: if `conda-lock` is not on PATH, fail with instructions to add it to `environment.yml` and run `pyve init --force --no-lock` (Story N.bf.7 — the `--no-lock` is required since the rebuild is bootstrapping the locker). Note: since N.bf.11 the default micromamba scaffold already declares `conda-lock`, so freshly-scaffolded projects reach `pyve lock` with the locker already installed and never hit this message.
...
| `PYVE_NO_LOCK` | Set to `1` for `--no-lock` semantics: don't use a lock this run (resolve from `environment.yml`, ignore any present lock without deleting it), skip the requirement (beats `--strict`), and omit `conda-lock` from a fresh scaffold |
