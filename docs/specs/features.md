...
- **Per-env `.state` file (Stories M.h.1, M.m):** each `.pyve/testenvs/<name>/` carries a sibling `.state` recording `backend`, `manifest`, `manifest_sha256`, `provisioned_at`, `last_used_at`. Plain `key=value`, sourceable. Written by `ensure_testenv_exists` / `_testenv_init_conda` on env creation (M.m); `last_used_at` is touched by `pyve test` on the success path (M.m). Consumed by future stories (M.p's `pyve testenv list` / `prune` read `provisioned_at` / `manifest_sha256` for staleness and unused-since reports).
...
- **`pyve test [--env <name>]` (Stories M.c, M.e v2.7.1, M.m).** **Pre-M.m:** `--env` accepted only `root` and `testenv`. **M.m extends the resolver** to accept any name declared in `[tool.pyve.testenvs]`. Behavior:
    ...
    - `--env <declared-name>` resolves the env via `resolve_testenv_path <name>`, ensures it exists (auto-creates if needed via `ensure_testenv_exists`), and execs pytest inside its venv. Conda-backed envs are rejected — `pyve testenv run` is venv-only (M.k) and the same gate applies; use `--env root` against a conda main env, or `micromamba run -p <path> pytest` as a manual fallback. **Lazy envs** (`lazy = true`) that have not yet been provisioned hard-error with a `pyve testenv install <name>` hint (M.n replaces this with auto-provisioning).
...
- **Silent-skip advisory (Story M.c)**: when `pyve test` routes to the testenv (default) **and** the root env has pytest importable, pyve prints a one-line advisory pointing at `--env root` before running pytest. This surfaces the bundled-env trap at invocation time rather than letting a mass-SKIP masquerade as a clean run. The advisory is non-fatal and does not fire for a normal repo checkout (whose root env has no pytest). Suppressible via `PYVE_NO_TESTENV_ADVISORY=1` for users who keep pytest in the root env deliberately.
