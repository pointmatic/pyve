...
- **Per-env `.state` file (Story M.h.1):** each `.pyve/testenvs/<name>/` carries a sibling `.state` recording `backend`, `manifest`, `manifest_sha256`, `provisioned_at`, `last_used_at`. Plain `key=value`, sourceable. Consumed by future stories (M.o touches `last_used_at`; M.p's `pyve testenv list` / `prune` read `provisioned_at` / `manifest_sha256` for staleness and unused-since reports).
...
- **`pyve test [--env root|testenv]` (Story M.c; renamed in M.e v2.7.1)**: `--env testenv` (default) preserves the historical behavior (run pytest in `.pyve/testenvs/testenv/venv/` — was `.pyve/testenv/venv/` pre-M.h). `--env root` routes pytest to the project's root env (delegates to `run_command python -m pytest`), the first-class form of the `pyve run python -m pytest` workaround. This exists for environments built from a bundled `environment.yml` that carry **both** pytest and the stack-under-test in the root env — where the default testenv is stack-less and `importorskip`-guarded tests would silently SKIP. An invalid `--env` value is a hard error. **Legacy `--env main` (M.c v2.7.0) hard-errors with a precise rename hint** per the Category-B deprecation-removal policy — no silent delegation.
