...
1. **Command surface:** ~~`pyve heal`~~ vs. `pyve check --fix` (or both, one delegating). Naming decides where confirm-flow and reporting live. **Resolved (Story P.ab.1, 2026-07-10): `pyve check --fix` — no separate `pyve heal` command.** Rationale: healing is check's verdicts acted upon — one diagnostic surface that detects, then (opted-in) repairs, keeps detection and repair vocabulary in one place and adds no new top-level command to learn. The confirm-flow and reporting live in the heal engine (`lib/heal.sh`), invoked by the check composer after the diagnostic sections. `--yes` assents to the batch per the P.l.1 flag semantics; non-TTY without `--yes` is report-only.
...
3. **Staleness network model:** opt-in vs. opt-out, timeout, cache TTL + location, flag/env-var names. **Resolved (Story P.ac integration spike, 2026-07-10) — measured on live endpoints and all failure classes; Story P.ad implements against this record.**
   - **Fetch pattern (proven):** `curl -fsSL --connect-timeout 2 --max-time 5 <url> 2>/dev/null || true` — empty stdout on ANY failure, pipeline rc always 0, stderr fully silent. The `2>/dev/null` is load-bearing: `-s` alone still emitted 49–85 bytes of error text per failure. Raw curl rcs for the record: DNS 6, refused 7, blackholed 28 (connect-timeout-bound), HTTP-4xx 22/56 — all non-zero, normalized by `|| true`.
   - **Worst-case wall-time (measured):** happy path 190–270 ms/endpoint; DNS failure 341 ms; refused 34 ms; blackholed host 2.05 s (bounded exactly by `--connect-timeout 2`); theoretical ceiling `--max-time` = 5 s/fetch (slow-drip server). Two sequential probes ≈ 4.1 s observed worst / 10 s ceiling — paid at most once per TTL window.
   - **Endpoints (decided):** project-guide → `https://pypi.org/pypi/project-guide/json` (189 ms, 126 KB, `"version"` extractable with grep/cut — no jq; the simple-API JSON is 73 KB, not worth the divergence). pyve → the **raw tap formula** `https://raw.githubusercontent.com/pointmatic/homebrew-tap/main/Formula/pyve.rb`, parsing `refs/tags/v<ver>` from its `url` line (returned the current v3.1.0). The GitHub releases API is **rejected**: it reports v1.13.3, the last cut GitHub Release — the release workflow deliberately changed after v1.13.3 (tags are now created on branches and merged to main; Release objects are no longer cut), so the API reflects the retired workflow's last artifact, not the current version. A merge-triggered automation that also creates a GitHub Release would make the API current again (viable if ever wanted), but it adds upstream automation and the API still burns the 60/hr anonymous rate limit; raw.githubusercontent.com is CDN-served with no API limit and is already current with no workflow change.
   - **Network model (decided): opt-out.** The probe runs by default for interactive human `check` runs and is suppressed by any of: `--offline` flag / `PYVE_NO_NETWORK=1` (equivalent surfaces, mirroring `--verbose`/`PYVE_VERBOSE`); the `CI` env var (CI gets no probe — staleness hints are for humans, and this keeps CI deterministic and rate-limit-free); an unexpired cache.
   - **Cache (decided):** one single-line version file per tool at `${XDG_CACHE_HOME:-$HOME/.cache}/pyve/latest/<tool>`, freshness by file mtime, **TTL 24 h**. A failed fetch never overwrites a cached value; an expired cache + offline → no hint, silently (info-only means never a stale-scare).
   - **Exit-code invariant (proven):** with the pattern above a network failure cannot change `check`'s exit code — the probe yields an empty string and the staleness line simply doesn't render.
...
5. **Heal vs. upgrade boundary:** does heal ever apply a version upgrade, or strictly repair broken state and defer upgrades to the staleness hints? **Resolved (Story P.ab.1, 2026-07-10): strictly repair — heal never applies a version upgrade.** A healthy-but-stale tool is Story P.ad's info-only staleness hint; `--fix` acts only on *broken* state (dead interpreter, dead wrapper, dangling shim, orphaned tree). Never-provisioned hosting is likewise not a fault (optional by contract → hint, not repair). The one deliberate nuance: repairing a dead hosted project-guide reinstalls through the provision machinery, which installs the current floor version — a side effect of repair-by-reinstall, not an upgrade path.
