...
### Story N.aw: F2 (revised) — Host project-guide as a Pyve-managed global tool [Blocked]
...
**Cross-repo gating — BLOCKED on `project-guide` v2.13.0.** The project-guide-side contract lives in [project-guide-requests/pyve-toolchain-hosting.md](project-guide-requests/pyve-toolchain-hosting.md) (authored here; see task 1). The contract changes are **in progress upstream and ship as `project-guide` v2.13.0** (confirmed 2026-06-05, dropping soon). The pyve-side consumption (tasks 2+) is **blocked until v2.13.0 is published**, per the "Cross-repo coordination with project-guide" essential; check back on the upstream release before unblocking. When it lands, this story pins `project-guide ≥ 2.13.0` and flips back to `[Planned]` to land the consumption. Task 1 (the request spec) is already done, so the heading carries `[Blocked]` rather than `[Planned]` to keep it out of the active work queue until the dependency clears.
...
- [ ] *(gated)* `pyve self install` installs/upgrades project-guide into the toolchain venv (best-effort, mirroring the toolchain-venv provisioning itself — build/install failure warns + continues).
- [ ] *(gated)* Create/refresh a `~/.local/bin/project-guide` shim → `<toolchain-venv>/bin/project-guide`; `pyve self uninstall` removes the shim (the toolchain-tree removal already drops the package).
- [ ] *(gated)* `DEFAULT_PYTHON_VERSION`-bump reconcile: reinstall project-guide into the newly-keyed toolchain venv + re-point the shim during `pyve self install` (rides the existing `_self_prune_stale_toolchain_versions` path).
- [ ] *(gated)* Remove the per-project provisioning from the composed-init project-guide orchestration: the accept decision no longer writes `[env.root] backend = "venv"` nor materializes a venv; repoint or retire the per-project `install_project_guide` venv-targeting path so it stops pip-installing into the project env.
- [ ] *(gated)* Reconcile the N.aj gate: `.project-guide.yml` presence no longer implies a project-local Python env, so it must no longer force the Python plugin active on a non-Python stack (this is what retires F3/N.ax). Update the project-essentials `.project-guide.yml` entry wording (global hosting, not "installed via pip into a venv-backed `root` `utility` env") at the same time.
- [ ] *(gated)* Tests: Node-only accept → project-guide hosted in the toolchain venv + shim present + `.project-guide.yml` scaffolded, **no** project `.venv` / utility-root venv; Python app present → its `.venv` reporting unchanged; decline → no shim/hosting changes; min-version pin (`project-guide ≥ 2.13.0`) enforced.
