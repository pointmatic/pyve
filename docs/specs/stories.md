...
- [ ] **Pre-existing integration-test instability (surfaced in N.al).** Triage/stabilize the `test_reinit.py` failures that predate N-4: **3 are deterministic on clean `main`** (`test_force_purges_existing_venv`, `test_force_prompts_for_confirmation`, `test_backend_conflict_in_interactive_mode` — text/`stderr`-routing expectation mismatches), and **2 are flaky** (`test_interactive_option_2_purges`, `test_force_with_missing_venv` — a non-deterministic asdf "Install Python `<ver>` now?" prompt fires even when the version is installed, so the non-interactive run cancels → exit 1). These are NOT composition regressions (proven via a stash → clean-`main` comparison in N.al), but the sweep is the right place to make the reinit suite deterministic — e.g. pin/pre-resolve the interpreter in the fixture so the asdf install prompt never fires, and align the assertions with current prompt text / output stream. If a fix balloons, split it to its own test-hardening story rather than blocking the sweep.
...
- story numbers embedded in test filenames: practical for cleanly partioning and attributing code and tests to the work done, but meaningless in the long term. Example `tests/unit/test_n_n_python_plugin.bats` should be refactored to `tests/unit/test_python_plugin.bats`. 
- story references in code: practical for tracking progress and understanding the context of the code, but meaningless in the long term. Example `# story N-N` should be justified as a critical cross-reference or removed. It would be better to relocate the essential context that is contained in a story into the code comments than a brittle, arbitrary story number that only loosely references the story (that may or may not remain in the documentation). 
