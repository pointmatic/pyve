...
**Note:** stale `.pyve/config` references also linger in `lib/` *code comments* (e.g. [init_composer.sh:35](../../lib/init_composer.sh#L35), several in [plugin.sh](../../lib/plugins/python/plugin.sh)); sweeping those is a code-comment cleanup outside this docs+verify story's scope and was not in the P.i.9–P.i.13 removal scope either — a candidate for a follow-up tidy story.

**This closes the P.i bundle** (`pyve.toml` is the sole config source; `.pyve/config` retired). Next `[Planned]` in Subphase P-1 is **Story P.j** (explicit-by-construction manifest + easy-mode wizard).
