...
### Story N.aw.2: Retire per-project provisioning + N.aj gate reconciliation [Planned]
...
- [ ] Remove the per-project provisioning from `run_project_guide_orchestration` ([lib/project_guide.sh](../../lib/project_guide.sh)): drop the host-env resolution + the `install_project_guide` call into the project env; scaffold (`project-guide init|update --no-input`) via the **global** `project-guide` (the N.aw.1 shim on PATH) rather than `<env_path>/bin/project-guide`. Guard with a clear "run `pyve self install`" message when `project-guide` is not globally resolvable. Repoint or retire the now-unused per-project `install_project_guide` venv-targeting path (and `run_project_guide_{init,update}_in_env`'s `<env_path>/bin` invocation).
- [ ] Reconcile the N.aj gate ([lib/plugins/python/plugin.sh](../../lib/plugins/python/plugin.sh) `python_plugin_is_active_in_project`): `.project-guide.yml` presence no longer implies a project-local Python env, so it must no longer force the Python plugin active on a non-Python stack (this is what retires F3/N.ax). Update [tests/unit/test_n_aj_python_active_gate.bats](../../tests/unit/test_n_aj_python_active_gate.bats) and the project-essentials `.project-guide.yml` entry wording (global hosting, not "installed via pip into a venv-backed `root` `utility` env") at the same time.
- [ ] Tests: Node-only accept → scaffolds `.project-guide.yml` via the global `project-guide`, **no** project `.venv` / utility-root venv installed; Python app present → its `.venv` reporting unchanged; decline → no hosting/scaffold changes. Preserve the G.* orchestration behavior the suite already pins.
...
### Story N.az.1: Toolchain PyYAML provisioning + `pyve_env_spec_helper.py` [Planned]
...
- [ ] Provision PyYAML into the toolchain venv: extend `pyve self install` (the `_self_install_toolchain_python` neighborhood, [lib/commands/self.sh](../../lib/commands/self.sh)) to `pip install pyyaml` into the toolchain venv — best-effort (failure warns + continues, like the toolchain-Python provisioning itself). Removal rides the toolchain-tree `rm -rf` in `pyve self uninstall` (no extra step). Add a `pyve_toolchain_has_pyyaml`-style availability check.
- [ ] New `lib/pyve_env_spec_helper.py` (copyright/license header; stdlib + `import yaml`): extract the first ` ```yaml ` fenced block under the `## 4.`/`### 4.0` heading (the machine surface per [wizard-env-contract.md](project-guide-requests/wizard-env-contract.md) §B — §4.1 table + §5–§9 prose ignored), parse it, project each env to the projectable subset (`purpose`/`backend`/`default`/`path`/`languages`/`frameworks`/`packaging`), default-fill missing optional fields, emit JSON on stdout. No closed-set validation (permissive — F6 backfills).
- [ ] Bash seam ([lib/commands/env.sh](../../lib/commands/env.sh) or [lib/envs.sh](../../lib/envs.sh)): invoke the helper via `pyve_toolchain_python` (per the toolchain-python essential — `py="$(pyve_toolchain_python 2>/dev/null)" || py="${PYVE_PYTHON:-python}"`); a clean "PyYAML unavailable → run `pyve self install`" error when `import yaml` fails (distinct exit-code contract from a parse error).
- [ ] Tests: gitbetter §4.0 fixture → correct projected JSON (root/testenv/sandbox, incl. `backend=none` advisory passthrough); missing-optional-field default-fill; absent spec file; PyYAML-absent error path (distinct exit code/message). Python helper via `pyve test`; provisioning + seam via bats.

### Story N.az.2: `pyve env sync` command + `pyve check` diff surface [Planned]
...
- [ ] `pyve env sync` subcommand ([lib/commands/env.sh](../../lib/commands/env.sh)): discover the spec (`project_guide_env_spec_path`, N.ay) → parse + project (N.az.1) → permissive validation stub (accept-all; clearly marked `# F6/N.ba: replace with closed-set trichotomy`) → stateless live diff vs the current `pyve.toml` (no baseline — `pyve.toml` *is* the baseline) → present → `[Y/n]` confirm (default `Y`).
- [ ] Apply writes `pyve.toml` only — does **not** materialize. Destructive diffs (dropping an `[env.*]` whose env exists on disk; a `backend` flip implying rebuild) default `N` / require explicit confirm.
- [ ] `pyve check` surfaces a non-empty live diff at **warn** severity (exit 0) — a spec-ahead project is a legitimate steady state.
- [ ] Tests against fixture §4 docs: additive diff (default `Y` → written), advisory backend (`none`/`homebrew` → written-not-materialized), destructive diff (default `N`), clean (no diff → no-op), plus the `pyve check` warn surface. (Unknown-value → hard-error is **F6/N.ba**'s test, not here — F4 accepts permissively.)
