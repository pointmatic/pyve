...
The tag was cut but the formula never updated — the publish was blocked. **Root cause is upstream, not a pyve regression:** Homebrew ≥ 5.1.15 (PR Homebrew/brew#22470, late May 2026) added tap-trust enforcement — loading a formula runs Ruby from its tap, so Homebrew now refuses non-official ("untrusted") taps unless explicitly trusted, and the action's `main.rb` calls `Formula["pointmatic/tap/pyve"]`. This becomes the **default** at Homebrew 6.0.0 / 5.2.0 (whichever lands first); the trigger was simply the runner's bundled `brew` crossing the 5.1.15 line. No local/unit reproduction exists — the failure mode is purely a function of the runner's `brew` version, a different axis than the v3-command-surface audit below. The other workflows ([test.yml](../../.github/workflows/test.yml), [deploy-docs.yml](../../.github/workflows/deploy-docs.yml)) only `brew install` official homebrew-core formulae, so the same class of failure can't surface there.

**Resolution arc.** First attempted the forward-compatible `brew trust` pre-step, which dead-ended in CI over two iterations: (1) `brew: command not found` — `brew` isn't on `PATH` in a custom `run:` step (the action adds it only for its own later steps), fixed with `eval "$(/home/linuxbrew/.linuxbrew/bin/brew shellenv)"`; then (2) `Error: Broken pipe` — `brew trust` is **interactive-by-design** (a human reviews the tap's Ruby before trusting), has **no non-interactive flag** (manpage confirms only `--tap`/`--formula`/`--cask`/`--command`/`--json`), and broken-pipes in CI's no-TTY shell. There is no documented way to run `brew trust` non-interactively (ddev hit the same wall and sidestepped it via fully-qualified `brew install` — N/A here, since the bump action *loads* rather than installs). **Landed fix:** drop the pre-step; set `HOMEBREW_NO_REQUIRE_TAP_TRUST=1` on the action step — Homebrew's documented CI mechanism for the transition window.
