...
**As of v3.1 (Subphase P-1, the P.i bundle), the v2 `.pyve/config` file is fully retired: pyve neither writes nor reads it.** `manifest_load` on a project with no `pyve.toml` returns the empty-config baseline — a legacy `.pyve/config`-only (unmigrated v2) project is therefore *uninitialized* from pyve's point of view. The **only** two code paths that still touch `.pyve/config` are the migrator (`self migrate` in [`lib/commands/self.sh`](../../lib/commands/self.sh)) and the opportunistic legacy-layout mover (in [`lib/envs.sh`](../../lib/envs.sh)), both of which read the legacy file solely to convert an old project forward. There is no v3.0 read-compat synthesis anymore (it was removed in P.i.13). A `pyve.toml`-only project is fully functional across `status` / `check` / `run` / `lock` / `env` (verified end-to-end in P.i.14).
...
### The v2 → v3 deprecation surface: migrator + soft banner (read-compat retired)

Two coordinated surfaces handle the v2 → v3 transition. Don't add a third ad-hoc nudge; don't conflate them.

1. **Deterministic migrator** — `pyve self migrate` ([`lib/commands/self.sh`](../../lib/commands/self.sh)). Writes `pyve.toml` from legacy sources and moves them into `.pyve/.v2-legacy/`. Idempotent; `--dry-run` / `--no-rebuild` flags expose intermediate states. (Migrate no longer rebuilds envs — `pyve init --force` is the separate, developer-run rebuild step; see [[A forced/refresh rebuild honors the manifest backend — `pyve.toml` outranks the filesystem heuristic, and `pyve self migrate` no longer rebuilds]].)
2. **v2 soft banner** — pre-dispatch hook in [`pyve.sh`](../../pyve.sh)'s `main()` (`_pyve_maybe_show_v2_banner`), which detects a legacy project via `_self_migrate_detect_v2_sources` (presence of `.pyve/config`). One-shot per `(PPID, cwd)` via a sentinel under `${XDG_STATE_HOME:-$HOME/.local/state}/pyve/`. Skips `--help` / `--version` / `--config` / `self` namespace. Suppressible via `PYVE_QUIET=1`.

The original v3.0 plan carried a **third** layer — a read-compat synthesis (`_manifest_synthesize_from_legacy` in [`lib/manifest.sh`](../../lib/manifest.sh)) that let an unmigrated v2 project keep working by synthesizing a v3 manifest shape from `.pyve/config`. **That layer was removed in Subphase P-1 (Story P.i.13).** A legacy `.pyve/config`-only project is now treated as *uninitialized* — `manifest_load` returns the empty-config baseline. The soft banner still fires (it detects `.pyve/config` directly, independent of the deleted synthesis), so an unmigrated user is still nudged toward `pyve self migrate`, which remains the one supported forward path.

**Why:** the two live surfaces cover the two remaining user states — (a) ready to migrate (uses `self migrate`), and (b) not yet migrated (sees the banner once per shell, and the project reads as uninitialized until they migrate). The read-compat "keep working without migrating" grace period was intentional for the v3.0 window and ended with v3.1; carrying it further would have kept two config readers permanently in sync for a transitional convenience. A third nudge inside any individual command (`pyve check`, `pyve status`, …) would either duplicate the banner's signal or fight its memoization.
...
### A forced/refresh rebuild honors the manifest backend — `pyve.toml` outranks the filesystem heuristic, and `pyve self migrate` no longer rebuilds
...
Two coupled invariants ship alongside it:

- **`pyve self migrate` does not rebuild.** It seeds `pyve.toml` + backs up legacy sources to `.pyve/.v2-legacy/` and stops; `pyve init --force` is the separate, developer-run next step. `--no-rebuild` is an accepted no-op (it now describes the only behavior).
...
**Why:** caught migrating a micromamba `root` project — `pyve init --force` → "Install pip dependencies?" → `bad interpreter: No such file or directory` for `.pyve/envs/root/conda/bin/pip`. The v3 layout mover had `mv`'d the flat prefix `.pyve/envs/<configured>/` → `.pyve/envs/root/conda/`, leaving 23 dead-shebang console scripts; `pyve init --force` then saw the directory and printed "environment already exists, skipping creation", so nothing healed it. This is the canonical existence-≠-runnability trap, in the migration path; it is the companion to the manifest-honoring rebuild ([[A forced/refresh rebuild honors the manifest backend — `pyve.toml` outranks the filesystem heuristic, and `pyve self migrate` no longer rebuilds]]) — together they were why migrating a micromamba project was unsafe.
